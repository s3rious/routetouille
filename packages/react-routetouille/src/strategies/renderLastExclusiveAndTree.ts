...
function findLastIndex<
  ...
  SearchArray extends Item[],
>(array: SearchArray, searchKey: string, searchValue: unknown): number {
...
function renderLastExclusiveAndTree<
  Route extends {},
...
>(router: Router, active: Route[]): ReactElement | null {
  ...
  let componentRoutes = active.filter(
    isWithReactComponent,
  ) as ReactComponentRoute[];
  const lastExclusiveIndex: number = findLastIndex<
    ReactComponentRoute,
    ReactComponentRoute[]
  >(componentRoutes, "exclusive", true);

  if (lastExclusiveIndex > -1) {
    componentRoutes = componentRoutes.slice(
      lastExclusiveIndex,
      componentRoutes.length,
    );
  }
