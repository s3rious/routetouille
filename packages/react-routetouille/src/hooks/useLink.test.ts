...
describe('useLink', () => {
  ...
  afterEach(() => {
    (useRouterModule.useRouter as any).mockReset();
    mockRouter.urlTo.mockReset();
    mockRouter.goTo.mockReset();
  });

  it('returns correct href when router and to are provided', () => {
    (useRouterModule.useRouter as any).mockReturnValue(mockRouter);
    const { result } = renderHook(() => useLink<string, { id: number }>({ to: 'foo', params: { id: 1 } }));
    expect(result.current.href).toBe('/foo');
  });

  it('returns hrefProp if router or to is missing', () => {
    (useRouterModule.useRouter as any).mockReturnValue(undefined);
    const { result } = renderHook(() => useLink<string, { id: number }>({ href: '/bar' }));
    expect(result.current.href).toBe('/bar');
  });

  it('handleClick calls goTo and prevents default', async () => {
    const goTo = vi.fn();
    (useRouterModule.useRouter as any).mockReturnValue({ ...mockRouter, goTo });
    const { result } = renderHook(() => useLink<string, { id: number }>({ to: 'foo', params: { id: 1 } }));
    const event = new MouseEvent('click', { bubbles: true, cancelable: true }) as unknown as React.MouseEvent<HTMLAnchorElement>;
