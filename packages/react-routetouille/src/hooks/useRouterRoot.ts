...
import { type RouterInterface, isGoToState } from "routetouille";
...
function useRouterRoot<Route extends {}, Router extends AbstractRouter<Route>>(
  ...
  const handleHistoryChange = useCallback(
    (_pathname: string, state: unknown) => {
  ...
  const handleAfterActivate = useCallback(
    () => setActive(router.active),
    [router.active],
  );
