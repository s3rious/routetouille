...
describe("BrowserHistory", () => {
  ...
  it("push", () => {
    ...
    expect(globalThis.history.pushState).lastCalledWith(
      { scrollTop: 100 },
      "",
      "/foo/?bar",
    );
  ...
  it("replace", () => {
    ...
    expect(globalThis.location.pathname).toEqual("/foo/");
    expect(globalThis.location.search).toEqual("?bar");
    expect(globalThis.history.replaceState).lastCalledWith(
      { scrollTop: 100 },
      "",
      "/foo/?bar",
    );
  ...
  it("emitter", () => {
    ...
    // biome-ignore lint/suspicious/noGlobalAssign: It's a mock for test
