...
function getRoutesTrackByActivator(
  map: RouteMap,
  activator: Activator,
): AbstractRoute[] | undefined {
...
function getFinalRouteByActivator(
  map: RouteMap,
  activator: Activator,
): RouteMapRoute | undefined {
...
function getActivatorByPartialPath(
  map: RouteMap,
  activator: RouteMapKey,
): string | undefined {
  return [...map.keys()].find((key) => {
    const keyParts = key.split(".");
    const activatorParts = activator.split(".");

    return activatorParts.every((activatorParts) =>
      keyParts.includes(activatorParts),
    );
  });
}

function getClosestFallbackByActivator(
  map: RouteMap,
  activator: RouteMapKey,
): RouteMapRoute | undefined {
...
function getRoutesTrackByFinalRoute(
  map: RouteMap,
  finalRoute: RouteMapRoute,
): AbstractRoute[] {
...
function getRoutesToMount(
  currentActive: AbstractRoute[],
  nextActive: AbstractRoute[],
): AbstractRoute[] {
  const sameBetweenCurrentAndNext = currentActive.filter((route) =>
    nextActive.includes(route),
  );
  return nextActive.filter(
    (route) => !sameBetweenCurrentAndNext.includes(route),
  );
}

async function performActionToRoutes(
...
>(createRouter?: (options: ComposedOptions) => ComposedInterface) {
  return (
    options: WithActiveOptions & ComposedOptions,
  ): WithActiveInterface & ComposedInterface => {
    ...
    async function activate(
      ...
      const isOptimistic: boolean = Boolean(
        typeof optimistic === "boolean" ? optimistic : options.optimistic,
      );
      ...
      if (nextActive != null) {
        ...
        const routesAfterMount = await performUnmountMount(
          currentActive,
          nextActive,
        );
