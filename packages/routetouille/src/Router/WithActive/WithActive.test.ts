...
import { WithActive, type WithActiveInterface } from "./WithActive.js";
import {
  WithRoot,
...
describe("`WithActive` router", () => {
  const Router = WithActive<
    WithMapOptions & WithRootOptions,
    WithMapInterface & WithRootInterface
  >(WithMap(WithRoot()));
  ...
  describe("methods", () => {
    ...
    const getActiveRoutesName = (router: any): any =>
      router.active.map((route: AbstractRoute) => route.name);
    ...
    let optimisticRouter: WithActiveInterface &
      WithMapInterface &
      WithRootInterface;
    ...
    beforeEach(() => {
      ...
      admin = Route({
        ...
        redirects: [
          [
            async () => !allowedToVisitAdmin,
            async () => await router.activate("login", false),
          ],
        ],
      });
      ...
      optimisticAdmin = Route({
        ...
        redirects: [
          [
            async () => !allowedToVisitAdmin,
            async () => await optimisticRouter.activate("login", false),
          ],
        ],
      });
      ...
      describe("`activator`", () => {
        ...
        describe("array of strings", () => {
          ...
          it("unmatched", async () => {
            ...
            expect(getActiveRoutesName(anotherRouter)).toEqual([
              "root",
              "foo",
              "404",
            ]);
