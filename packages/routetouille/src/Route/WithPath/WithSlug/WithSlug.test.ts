...
import { WithSlug, type Slug } from "./WithSlug.js";
    ...
    describe("match", () => {
      ...
      describe("params", () => {
        it("matches and returns matched params", () => {
          expect(WithSlug()({ path: ":id/" }).match("42/")).toEqual({
            id: "42",
          });
          expect(WithSlug()({ path: "foo-:id/" }).match("foo-42/")).toEqual({
            id: "42",
          });
          expect(WithSlug()({ path: ":foo-:bar/" }).match("baz-quz/")).toEqual({
            foo: "baz",
            bar: "quz",
          });
        });
