...
function extractParams(path: string, part: string): Params | false {
  const paramNames: string[] = [...(path.match(/:\w+/g) ?? [])].map((param) =>
    param.replace(/:/, ""),
  );
  const pathRegexpPattern = paramNames.reduce(
    (pattern, paramName) =>
      pattern.replace(new RegExp(`:${paramName}`), "(\\w+)?"),
    `^${path.replace(/^\?/g, "\\?")}$`,
  );
  ...
  return matches.reduce((object, paramMatch, index) => {
    object[paramNames[index]] = paramMatch;
    return object;
  }, {} as Params);
}

export { extractParams, type Params };
