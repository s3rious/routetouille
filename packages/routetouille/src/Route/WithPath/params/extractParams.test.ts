...
const queryStringsNonFirst: Array<
  [path: string, part: string, result: Result]
> = [
  ["&foo", "&foo", false],
  ["&foo=bar", "&foo=bar", false],
  ["&:id", "&34", { id: "34" }],
...
describe("hasParams", () => {
  it("matches properly", () => {
    paths.forEach(([path, part, result]) => {
      expect(JSON.stringify(extractParams(path, part))).toEqual(
