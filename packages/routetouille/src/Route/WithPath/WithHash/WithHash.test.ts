...
import { WithHash, type Hash } from "./WithHash.js";
    ...
    describe("match", () => {
      ...
      describe("params", () => {
        it("matches and returns matched params", () => {
          expect(WithHash()({ path: "#:id" }).match("#42")).toEqual({
            id: "42",
          });
          expect(WithHash()({ path: "#foo-:id" }).match("#foo-42")).toEqual({
            id: "42",
          });
          expect(WithHash()({ path: "#:foo-:bar" }).match("#baz-quz")).toEqual({
            foo: "baz",
            bar: "quz",
          });
        });
