...
import { WithSearch, type Search } from "./WithSearch.js";
...
describe("`WithSearch` route", () => {
  ...
  describe("methods", () => {
    describe("match", () => {
      ...
      describe("params", () => {
        it("returns false if not matches", () => {
          expect(WithSearch()({ path: "?:id" }).match("?42")).toEqual({
            id: "42",
          });
          expect(WithSearch()({ path: "?foo-:id" }).match("?foo-42")).toEqual({
            id: "42",
          });
          expect(WithSearch()({ path: "?foo=:id" }).match("?foo=42")).toEqual({
            id: "42",
          });
          expect(WithSearch()({ path: "?:id=bar" }).match("?42=bar")).toEqual({
            id: "42",
          });
          expect(
            WithSearch()({ path: "?:foo=:bar" }).match("?baz=quz"),
          ).toEqual({ foo: "baz", bar: "quz" });
        });
      });
    });
  });
